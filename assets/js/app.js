(() => {
  ...
  async function aboutView() {
    ...
    const focusCard = f => (f && ((f.items && f.items.length) || f.description))
      ? `<article class="focus-card"><span class="focus-ic">${icon(f.icon || 'target')}</span><div><h3>${esc(f.title || '')}</h3>${f.items && f.items.length ? `<ul>${f.items.map(i => `<li>${esc(i)}</li>`).join('')}</ul>` : ''}${f.description ? `<p>${esc(f.description)}${f.years ? ` ${esc(f.years)}` : (pv && ('years' in f) ? ' <span class="muted">[add years]</span>' : '')}</p>` : ''}</div></article>` : '';
    const fo = p.focus || {};
